...
## 2026-04-20
  ...
  + 서브쿼리 연산자(WHERE 뒤)
    ...
    - 집합 연산자
      - IN, NOT IN
    - 한정 연산자
      - ANY, SOME, ALL -> 결과값중 1개 결정 MAX / MIN
    - 존재 연산자
      -EXISTS, NOT EXISTS
  + 스칼라 서브쿼리
    - 컬럼 대신 사용
  ...
  + 인라인 뷰
    - 테이블 대신 사용
    - 한번 사용(보안,페이징) -> FROM 뒤에
    - 즉석 검색 결과
    - SELECT ~ FROM (SELECT ~)
    - 없는 데이터가 있는경우 오류가 발생 (SELECT 안에 컬럼이 있어야한다)
    - 스칼라 서브쿼리와 인라인 뷰는 저장이 안됨 (보안)

</details>

<details>
<summary>뷰 (View)</summary>
  
## 2026-04-20
   + 뷰 정의
     - 하나이상의 테이블을 연결해서 만든 가상테이블
     - 가상의 테이블이기때문에 저장이 안된다(보안)
     - 보여만 주는 역할
     - DML사용도 가능
   + 장점
   ...
   + 생성
     - CREATE VIEW view_name AS [SELECT ~] -> SELECT ~ 문장을 저장한다
   + 수정
     - CREATE OR REPLACE VIEW view_name AS SELECT ~ -> 있으면 수정하고 없으면 만든다
   + 삭제
     - DROP VIEW view_name
